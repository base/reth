...
use reth_provider::{
    BlockExecutionOutput, BlockNumReader, BlockReader, ChangeSetReader, DatabaseProviderFactory, DatabaseProviderROFactory, ExecutionOutcome, HashedPostStateProvider, ProviderError, PruneCheckpointReader, StageCheckpointReader, StateProvider, StateProviderFactory, StateReader, TrieDbTxProvider, TrieReader, providers::{OverlayStateProviderFactory, triedb::TrieDbOverlayStateRoot}
};
    ...
    Evm: ConfigureEvm<Primitives = N> + 'static,
{
        ...
        Evm: ConfigureEngineEvm<T::ExecutionData, Primitives = N>,
    {
            ...
            maybe_state_root
        {
            ...
            let (root, updates) = ensure_ok_post_block!(
                self.compute_state_root_triedb(block.parent_hash(), &hashed_state, ctx.state()),
                block
            );
    ...
    /// Compute state root for the given hashed post state using TrieDB.
    fn compute_state_root_triedb(
        &self,
        parent_hash: B256,
        hashed_state: &HashedPostState,
        state: &EngineApiTreeState<N>,
    ) -> ProviderResult<(B256, TrieUpdates)> {
        let (mut input, _) = self.compute_trie_input(parent_hash, state)?;
        ...
        let triedb_tx = self.provider.database_provider_ro()?.into_triedb_tx();
        let (root, trie_updates) = TrieDbOverlayStateRoot::new(triedb_tx, input).incremental_root_with_updates()?;
