use crate::{TrieDbTxProvider, providers::{TrieDbTransaction, triedb::TrieDbOverlayStateRoot}};
...
use reth_trie::{
    HashedPostStateSorted, KeccakKeyHasher, TrieInputSorted, hashed_cursor::{HashedCursorFactory, HashedPostStateCursorFactory}, prefix_set::TriePrefixSets, trie_cursor::{InMemoryTrieCursorFactory, TrieCursorFactory}, updates::{TrieUpdates, TrieUpdatesSorted}
};
use reth_trie_db::{
...
{
    pub fn compute_triedb_state_root(self, prefix_sets: TriePrefixSets) -> ProviderResult<(B256, TrieUpdates)> {
        let triedb_tx = self.provider.into_triedb_tx();
        let (root, trie_updates) = TrieDbOverlayStateRoot::new(triedb_tx, self.trie_updates, self.hashed_post_state, prefix_sets).incremental_root_with_updates()?;
