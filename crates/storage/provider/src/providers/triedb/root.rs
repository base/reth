...
pub struct TrieDbOverlayStateRoot {
    ...
    input_nodes: Arc<TrieUpdatesSorted>,
    input_state: Arc<HashedPostStateSorted>,
...
impl TrieDbOverlayStateRoot {
    ...
    fn calculate(self, _retain_updates: bool) -> Result<(B256, TrieUpdates), ProviderError> {
        let start_time = Instant::now();
        let (overlay_state, removed_keys, mut removed_storage_keys) =
            build_overlay_state(self.input_nodes, self.input_state, self.input_prefix_sets);
...
fn build_overlay_state(
    ...
    let mut overlay_mut = OverlayStateMut::with_capacity(
        input_nodes.account_nodes_ref().len() * 16 + input_nodes.storage_tries_ref().len() * 16,
    );
    for (key, branch) in input_nodes.account_nodes_ref() {
        ...
        for i in 0..16 {
            if let Some(branch) = branch &&
                branch.hash_mask.is_bit_set(i)
            {
                path.push(i);
                overlay_mut.insert(path.into(), Some(OverlayValue::Hash(branch.hashes[hash_idx])));
                hash_idx += 1;
                path.pop();
            }
        }
    }
    for (account, storage_updates) in input_nodes.storage_tries_ref() {
        let mut storage_prefix_set = input_prefix_sets.storage_prefix_sets.get_mut(account);
    ...
    for (account, storage_updates) in input_state.account_storages() {
        let account_path = Nibbles::unpack(account);
        for (key, value) in storage_updates.storage_slots_ref().into_iter() {
            overlay_mut.insert(
                RawPath::from(account_path).join(&Nibbles::unpack(key).into()),
...
mod tests {
    ...
    fn test_state_root_large_dataset() {
        ...
        for (hashed_addr, account_opt) in &hashed_state.accounts {
            commit_tx
                .set_account(*hashed_addr, *account_opt)
    ...
    fn test_state_root_with_storage_overlay() {
        ...
        initial_tx.set_account(hashed_address, Some(account)).expect("Failed to set account");
