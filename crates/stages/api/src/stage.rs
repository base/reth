...
mod tests {
    ...
    use reth_provider::{
        ProviderFactory, StaticFileProviderBuilder, StaticFileProviderFactory, StaticFileSegment, providers::{RocksDBProvider, TrieDbProvider}, test_utils::MockNodeTypesWithDB
    };
    ...
    fn test_exec_input_next_block_range_with_transaction_threshold() {
        ...
        let provider_factory = ProviderFactory::<MockNodeTypesWithDB>::new(
            ...
            TrieDbProvider::open(create_test_triedb_dir().0.keep()).unwrap(),
        )
        .unwrap();
